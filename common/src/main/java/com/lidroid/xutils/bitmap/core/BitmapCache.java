...
public class BitmapCache {
    ...
    public Bitmap downloadBitmap(String uri, BitmapDisplayConfig config, final BitmapUtils.BitmapLoadTask<?> task) {
        ...
        try {
            ...
            if (globalConfig.isDiskCacheEnabled()) {
                ...
                // chenbo modify 本地图片不放入cache
                if (mDiskLruCache != null && URLUtil.isNetworkUrl(uri)) {
                    try {
                        snapshot = mDiskLruCache.get(uri);
                        // chenbo add 如果只加载本地缓存，此处直接跳过网络加载
                        if (snapshot == null && !config.isOnlyLoadLocal()) {
            ...
            if (bitmap == null) {
                // chenbo add,如果加载本地图片，最好直接decodeFile，因为转换成byte[]太损耗内存
                if (uri.startsWith("/")) {
                    bitmapMeta.inputStream = new FileInputStream(uri);
                    bitmapMeta.expiryTimestamp = Long.MAX_VALUE;
                    bitmap = decodeBitmapMeta(bitmapMeta, config);
                } else {
    ...
    public Bitmap getBitmapFromMemCache(String uri, BitmapDisplayConfig config) {
        if (globalConfig.isMemoryCacheEnabled()) {
            if (mMemoryCache == null) {
                LogUtils.d("bacy->mMemoryCache is null, init it");
                initMemoryCache();
            }
            MemoryCacheKey key = new MemoryCacheKey(uri, config);
